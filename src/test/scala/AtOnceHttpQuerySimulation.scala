...
class AtOnceHttpQuerySimulation extends Simulation {
    ...
    object Conf {
        ...
        val queriesPerSecond = getInt("qps.rps", 10)
        ...
        //val queryUrl = getStr("qps.query.url", s"http://localhost:8764/api/apps/zola_registry/query/zola_registry")
        val queryUrl = getStr("qps.query.url", s"https://dg01.us-west1.dev.cloud.lucidworks.com/lucidworks/gpt/fusion/api/apps/zola_registry/query/zola_registry")
        val username = getStr("qps.fusion.user", "rohit.singh@lucidworks.com")
        val password = getStr("qps.fusion.pass", "Rahul642514*")
        val maxNumUsers = getStr("maxNumUsers", "5")
        ...
        def updateJwtToken() = {
            ...
            jwtToken = "lwauthsessionid=MWU2YWQwNDItYjRmOC00NDczLWJlZDQtNzc4ZDQ5YjgzNjU4"
    ...
    val qps = scenario("UsersAtOnce-" + Conf.queriesPerSecond).exec(Query.searchWithBasicAuth)

    setUp(
        qps.inject(
            atOnceUsers(Conf.maxNumUsers.toInt)).protocols(httpConf)
    ).maxDuration(Conf.testDurationMins minutes)
}