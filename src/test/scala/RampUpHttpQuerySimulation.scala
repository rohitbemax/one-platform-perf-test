...
class RampUpHttpQuerySimulation extends Simulation {
    ...
    object Conf {
        ...
        val usersPerSecond = getInt("usersPerSec", 2)
        val maxNumUsers = getInt("maxNumUsers", 50)
    ...
    //Scenario: injects users from starting rate1 to target rate2, defined in users per second, during a given duration.
    setUp(
    qps.inject(
        rampUsersPerSec(Conf.usersPerSecond.toInt) to Conf.maxNumUsers.toDouble during
            (Conf.testDurationMins minutes)).protocols(httpConf)
    )
}